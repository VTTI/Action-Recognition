...
def visualize(frames, annotations, csv_file, plate=plate_blue, max_num=5):
            ...
            for ann in anno:                
                ...
                for k, lb in enumerate(label):
                    if k >= max_num:
                        break                    
                    text = abbrev(lb)
                    csv_file.write(str(ind) + ","+str(dcount) + ","+str(text)+ ","+str(score[k]) +"," +'%f,%f,%f,%f,' % (box[0], box[1], box[2],box[3]) + "\n")
...
def parse_args():
    ...
    # config file
    parser.add_argument('--config', type=str, default='latest_spatiotemporal.yaml', help='Path to the config file for paths and hyperparameters')
...
def detection_inference(args, config, frame_paths):
    ...
    """
    model = init_detector(config['detConfig'], config['detCheckpoint'], args.device)
    ...
    for frame_path in frame_paths:
        ...
        result = result[0][result[0][:, 4] >= config['detThreshold']]
...
def main():
    ...
    frame_paths, original_frames = frame_extraction(args.input)
    ...
    mmcv_config = mmcv.Config.fromfile(config['configFile'])
    mmcv_config.merge_from_dict(args.cfg_options)
    val_pipeline = mmcv_config.data.val.pipeline
    ...
    timestamps = np.arange(window_size // 2, num_frame + 1 - window_size // 2,
                           config['predictStepsize'])

    # Load label_map
    label_map = load_label_map(config['labelMap'])
    try:
        if mmcv_config['data']['train']['custom_classes'] is not None:
            label_map = {
                id + 1: label_map[cls]
                for id, cls in enumerate(mmcv_config['data']['train']
                                         ['custom_classes'])
            }
    except KeyError:
        pass
    ...
    human_detections = detection_inference(args, config, center_frames)
    ...
    # Get img_norm_cfg
    img_norm_cfg = mmcv_config['img_norm_cfg']
    ...
    try:
        ...
        mmcv_config['model']['test_cfg']['rcnn']['action_thr'] = .0
    except KeyError:
        pass

    mmcv_config.model.backbone.pretrained = None
    model = build_detector(mmcv_config.model, test_cfg=mmcv_config.get('test_cfg'))
        ...
        with torch.no_grad():
            ...
            for i in range(len(result)):
                ...
                for j in range(proposal.shape[0]):
                    if result[i][j, 4] > config['actionThreshold']:
    ...
    dense_n = int(config['predictStepsize'] / config['outputStepsize'] )
    frames = [
    ...
    vis_frames = visualize(frames, results, csv_file)
